...
describe('Persistent Node Chat Server', function() {
  ...
  it('Should output all messages from the DB', function(done) {
    // Let's insert a message into the db
       // var queryString = `INSERT INTO messages (room, user, content) VALUES ("main", "anonymous", "Men like you can never change!");`
       //var queryArgs = [];
      ...
      function(){
        ...
        request({ 
          ...
          function(error, response, body) {
            ...
            if ( error ){ console.log("ERROR TEST:",error); }

            var messageLog = JSON.parse(body);
            expect(messageLog[0].content).to.equal('Men like you can never change!');
            expect(messageLog[0].room).to.equal(2);
            done();
        });
